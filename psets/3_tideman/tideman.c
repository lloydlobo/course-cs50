  ...
  for (int i = 0; i < voter_count; i++) {
    ...
    for (int j = 0; j < candidate_count; j++) {
      ...
      if (fgets(name, sizeof(name), stdin) == NULL) {
        ...
        // find new line
        // strchr()  (string function) replace the newline character in the
        // string with ‘\0’ if it exists.
...
/* If name is a match for the name of a valid candidate, then you should update
  ...
  preference (where 0 is the first preference, 1 is the second preference, etc.)
  Recall that ranks[i] here represents the user’s ith preference. The function
  should return true if the rank was successfully recorded, and false otherwise
  (if, for instance, name is not the name of one of the candidates). You may
